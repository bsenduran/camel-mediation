...
import org.wso2.carbon.camel.mediation.uri.URITemplate;
import org.wso2.carbon.camel.mediation.uri.URITemplateException;
...
public class CamelMediationEngine implements CarbonMessageProcessor {
    ...
    public boolean receive(CarbonMessage cMsg, CarbonCallback requestCallback) {
        ...
        Map<String, Object> transportHeaders =
                (Map<String, Object>) cMsg.getProperty(CamelMediationConstants.TRANSPORT_HEADERS);
        ...
        if (consumer != null) {
            ...
            exchange.setProperty(CamelMediationConstants.ORIGINAL_MESSAGE, cMsg);
            ...
            if (null == exchange.getException()) {
                ...
                if (!mediatedHeaders.isEmpty() && mediatedResponse != null) {
                    try {
                        ...
                        mediatedResponse.setProperty(CamelMediationConstants.HTTP_STATUS_CODE, statusCode);
                    } catch (ClassCastException classCastException) {
                        log.info("Response Http Status code is invalid. response code : " +
                                mediatedHeaders.get(Exchange.HTTP_RESPONSE_CODE));
                    }
                    mediatedHeaders.remove(Exchange.HTTP_RESPONSE_CODE);
                    mediatedResponse.setProperty(CamelMediationConstants.TRANSPORT_HEADERS, mediatedHeaders);
                }
            } else {
                mediatedResponse =
